...
본문은 두 가지 방식으로 편집합니다. 도구 모음의 **서식** 과 **MD** 버튼으로 전환하며, 어느 쪽으로 쓰든 저장되는 내용은 같은 Markdown 문서입니다.

- **서식**: 제목·목록 같은 구조가 화면에 그대로 반영되는 편집기입니다. Markdown 문법을 몰라도 도구 모음의 버튼으로 서식을 지정할 수 있습니다.
- **MD**: Markdown 원문을 직접 쓰고 오른쪽에서 렌더링 결과를 미리 봅니다. 표나 수식처럼 서식 편집기가 다루지 않는 문법을 쓸 때 적합합니다.

서식 편집기의 도구 모음은 세 묶음으로 나뉩니다.

| 묶음 | 버튼 | 설명 |
| --- | --- | --- |
| 실행취소 | ↩ ↪ | 직전 편집을 되돌리거나 다시 적용합니다. `Ctrl+Z`, `Ctrl+Y` 로도 동작합니다. |
| 블록 서식 | 제목·부제목·소제목·본문·목록·번호·체크 | 커서가 놓인 줄의 종류를 바꿉니다. |
| 인라인 서식 | B · I | 선택한 글자를 굵게 또는 기울임으로 만듭니다. `Ctrl+B`, `Ctrl+I` 로도 동작합니다. |
| 삽입 | ─ 구분선 | 커서 아래에 가로 구분선을 넣습니다. |

인라인 서식을 적용하면 글자 양쪽에 `**` 나 `*` 마커가 함께 표시됩니다. 마커를 지우지 않고 흐리게 남기는 방식이라서, 지금 어느 범위에 서식이 걸려 있는지 눈으로 확인하면서 편집할 수 있습니다. 강조 모양은 그 줄에서 커서를 옮기면 적용됩니다.

코드는 Markdown 의 코드 펜스, 곧 백틱 세 개로 여닫는 구간으로 감싸면 하나의 코드 상자로 묶입니다. 상자 안쪽 내용은 목록이나 제목으로 해석되지 않으므로 코드가 원문 그대로 보존됩니다. 상자 안에서는 Enter 가 줄바꿈으로 동작하고, 높이는 내용에 맞춰 늘어납니다. 언어를 적어 두었다면 상자 오른쪽 위에 표시됩니다.
