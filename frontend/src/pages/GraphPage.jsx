 ...
 * ──────────────────────────────────────────────────────────── */
const TAG_COLORS = [
  '#1a7a5e', '#2563eb', '#7c3aed', '#db2777', '#d97706',
...
function stepPhysics(nodeArr, edgeArr, W, H) {
  ...
  const idxMap = {}
  nodeArr.forEach((nd, i) => { idxMap[nd.id] = i })
  edgeArr.forEach(({ source, target }) => {
    const i = idxMap[source], j = idxMap[target]
...
function drawGraph(ctx, nodeArr, edgeArr, hoveredId, W, H) {
  ...
  const idxMap = {}
  nodeArr.forEach((n, i) => { idxMap[n.id] = i })
    ...
    <div className="graph-page">
      ...
      <div className="graph-header">
        <div className="graph-header-left">
          ...
          <span className="graph-meta">노트 {nodeCount}개 · 연결 {edgeCount}개</span>
        </div>
            ...
            <div className="graph-tooltip" style={{ left: x + 18, top: y - 10 }}>
              ...
              <div className="graph-tooltip-vitality">
                <span
                  ...
                  style={{ background: vitalityColor(vitality) }}
                />
                활력 {Math.round(vitality * 100)}%
              </div>
              {node.tags?.length > 0 && (
                <div className="graph-tooltip-tags">
                  {node.tags.map(t => <span key={t} className="graph-tooltip-tag">#{t}</span>)}
                </div>
              )}
